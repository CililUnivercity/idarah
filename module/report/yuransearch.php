...
<br>
<form class="form-horizontal" action="?page=report&&reportpage=yuransearch" method="POST">
...
<?php
    ...
    if($status_s == '1'){
            $sql = "SELECT COUNT(s.student_id),s.*,sr.*,p.* FROM students s 
                 INNER JOIN student_register sr ON s.st_id=sr.st_id
                 LEFT JOIN payments p ON sr.sr_id=p.sr_id
                 WHERE s.class='$class_s' and s.ft_id='$faculty_s' and s.dp_id='$department_s' and sr.academic_year='$year_s' and sr.term='$term_s'
                 ";
    }else{
            $sql = "SELECT COUNT(s.student_id),s.*,sr.*,p.* FROM students s 
                INNER JOIN student_register sr ON s.st_id=sr.st_id
                LEFT JOIN payments p ON sr.sr_id=p.sr_id
                WHERE s.class='$class_s' and s.ft_id='$faculty_s' and s.dp_id='$department_s' and sr.academic_year='$year_s' and sr.term='$term_s' and sr.pay_status='$status_s'
                ";
    }
    ...
    if($status_s == '1'){
        $sql = "SELECT s.*,sr.*,p.* FROM students s 
             INNER JOIN student_register sr ON s.st_id=sr.st_id
             LEFT JOIN payments p ON sr.sr_id=p.sr_id
             WHERE s.class='$class_s' and s.ft_id='$faculty_s' and s.dp_id='$department_s' and sr.academic_year='$year_s' and sr.term='$term_s' ORDER BY s.student_id $limit";
    }else{
        $sql = "SELECT s.*,sr.*,p.* FROM students s 
             INNER JOIN student_register sr ON s.st_id=sr.st_id
             LEFT JOIN payments p ON sr.sr_id=p.sr_id
             WHERE s.class='$class_s' and s.ft_id='$faculty_s' and s.dp_id='$department_s' and sr.academic_year='$year_s' and sr.term='$term_s' and sr.pay_status='$status_s' ORDER BY s.student_id $limit";
    }
    $query = mysqli_query($con, $sql);
    ...
    if($status_s == '1'){
        $sql_tr = mysqli_query($con, "SELECT s.*,sr.*,p.* FROM students s 
            INNER JOIN student_register sr ON s.st_id=sr.st_id
            LEFT JOIN payments p ON sr.sr_id=p.sr_id
            WHERE s.class='$class_s' and s.ft_id='$faculty_s' and s.dp_id='$department_s' and sr.academic_year='$year_s' and sr.term='$term_s'
            ");
...
<div class="pull-right">
    <a href="module/report/yuranprint.php?term=<?= $term_s ?>&&year=<?= $year_s ?>&&class=<?= $class_s ?>&&faculty=<?= $faculty_s ?>&&department=<?= $department_s ?>&&status=<?= $status_s ?>" target="_blank">
