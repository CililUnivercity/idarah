<?php
    ...
    if($ft_id == ""){
        if($dp_id == '22'){
            $dp_name = 'PAI';
            $student = mysqli_query($con, "SELECT s.*,p.* FROM students s INNER JOIN pretest p ON s.st_id=p.st_id WHERE dp_id='22' AND income_year='$rsAdmYear' AND class='$class' AND muqaddimah='1' ORDER BY s.gender,s.firstname_jawi");
        }else{
            $dp_name = 'PBSM';
            $student = mysqli_query($con, "SELECT s.*,p.* FROM students s INNER JOIN pretest p ON s.st_id=p.st_id WHERE dp_id='23' AND income_year='$rsAdmYear' AND class='$class' AND muqaddimah='1' ORDER BY s.gender,s.firstname_jawi");
        }
        
    }else{
        if($ft_id == '122'){
            $dp_name = 'SYA';
            $student = mysqli_query($con, "SELECT s.*,p.* FROM students s INNER JOIN pretest p ON s.st_id=p.st_id WHERE ft_id='122' AND income_year='$rsAdmYear' AND class='$class' AND muqaddimah='1' ORDER BY s.gender,s.firstname_jawi");
        }elseif($ft_id == '123'){
            $dp_name = 'USU';
